...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  try{renderer=new T.WebGLRenderer({antialias:true,alpha:false,powerPreference:'high-performance'});}catch{onError('此瀏覽器無法啟動 3D 檢視器。請使用已啟用 WebGL 的瀏覽器。');return;}
  ...
  renderer.domElement.setAttribute('aria-label','互動式人體解剖。拖曳可環繞、捏合或滾動可縮放，點選構造可查看詳細資訊。');
  ...
  const loadChunk=async(ci:number)=>{
   ...
   groups.forEach((gs,system)=>{const geometry=mergeGeometries(gs,false);if(!geometry)throw new Error('無法組合解剖模型的幾何資料。');geometries.push(geometry);const mesh=new T.Mesh(geometry,mats.get(system as never));mesh.frustumCulled=false;scene.add(mesh);});
   lastState=null;loaded++;onProgress(Math.round(loaded/atlas.chunks.length*100));dirty=true;
  };
  (async()=>{try{let cursor=0;await Promise.all(Array.from({length:3},async()=>{while(cursor<atlas.chunks.length){const i=cursor++;await loadChunk(i);}}));if(!disposed){ready=true;dirty=true;}}catch(e){if(!disposed)onError(e instanceof Error?e.message:'無法載入解剖模型。');}})();
  ...
  const contextLost=(e:Event)=>{e.preventDefault();onError('裝置已暫停 3D 工作階段，請重新載入以繼續。');};renderer.domElement.addEventListener('webglcontextlost',contextLost);
